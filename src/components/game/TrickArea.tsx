...
export function TrickArea({ trick, players, winnerId }: TrickAreaProps) {
  if (!trick || trick.plays.length === 0) {
    return (
      <div className="w-48 h-28 sm:w-64 sm:h-36 rounded-2xl border-2 border-dashed border-border/30 flex items-center justify-center">
        <span className="text-text-muted/50 text-xs">Zone de jeu</span>
      </div>
    );
  }
